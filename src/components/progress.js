...
const Progress = ({data}) => {
  ...
  {data.map(({frontmatter}) => {
    ...
    return (
      <><Flex onClick={goToUnit} sx={{cursor: 'pointer', gridColumn: 1, width: '100%', color: frontmatter.highlight, fontWeight: 'bold', lineHeight: 1.3}}>
      ...
      <Box onClick={goToUnit} sx={{cursor: 'pointer', gridColumn: 2, color: frontmatter.highlight, fontWeight: 'bold'}}>{Math.round(frontmatter.percent*100)}%</Box></>
    )
  })}
  </Grid>
  )
